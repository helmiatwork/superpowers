...
- **No codemap? Check Outline first, then run `cartography`.** If a local `CODEMAP.md` is missing, check Outline for the project's codebase map collection (see `codebase-context` skill). If Outline has maps, use those. If neither exists, delegate to @explorer to run the `cartography` skill, then have @librarian push results to Outline for future sessions.
- **Orchestrators must enforce this.** When delegating to ANY agent (@explorer, @fixer, @librarian, etc.), include in the briefing: "Read `CODEMAP.md` files before scanning directories. If local codemaps don't exist, check Outline for codebase maps. Do not read all files — use codemaps for structural understanding."
